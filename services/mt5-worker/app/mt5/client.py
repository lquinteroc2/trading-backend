...
class Mt5Client:
    ...
    def place_order(self, request: OrderRequest) -> Dict[str, Any]:
        ...
        if tick is None:
            raise Mt5NotConnectedError(f"No tick available for {request.symbol}")

        order_type = self._mt5.ORDER_TYPE_BUY if request.direction == "BUY" else self._mt5.ORDER_TYPE_SELL
        requested_price = request.entryPrice
        market_price = float(tick.ask if request.direction == "BUY" else tick.bid)
        order_payload = {
            ...
            "deviation": 20,
            "magic": 150015,
            "comment": request.comment or "INVERSIONES_LIVE_LIMITED",
            "type_time": self._mt5.ORDER_TIME_GTC,
            "type_filling": self._mt5.ORDER_FILLING_IOC,
        }
        result = self._mt5.order_send(order_payload)
        if result is None:
            raise Mt5NotConnectedError("MT5 order_send returned no result")

        retcode = getattr(result, "retcode", None)
        if retcode != self._mt5.TRADE_RETCODE_DONE:
            comment = getattr(result, "comment", "Broker rejected order")
            raise Mt5NotConnectedError(f"Broker rejected order: {comment} ({retcode})")

        executed_price = float(getattr(result, "price", market_price) or market_price)
        ...
        return {
            ...
            "spread": abs(executed_price - requested_price),
